...
@Component({
  selector: 'app-tablapartido',
  templateUrl: './tablapartido.component.html',
  styleUrls: ['./tablapartido.component.css']
})
export class TablapartidoComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, // formularios reactivos
    private tablapartidokService: TablapartidokService,
  ...
  sendMessage() { // enlace con bd envia infomracion 
    if (this.form.invalid)
      return //retorna un falso
        this.tablapartidokService.save(this.form.value).subscribe((res: any) => {
      if (res.status){
